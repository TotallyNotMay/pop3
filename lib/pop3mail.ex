defmodule Pop3 do
   ...
   ## Examples
     ...
     Download maximum 2 emails from the email account and also save the raw undecoded message.
       ...
       $ pop3_downloader --max=2 --raw --username=hendrik.lorentz@gmail.com --password=secret --output=mailbox

       or

       iex(1)> Pop3.cli(["--max=2", "--raw", "--username=hendrik.lorentz@gmail.com", "--password=secret", "--output=mailbox"])
   ...
   * `username`  - email account name. Gmail users can precede the name with 'recent:' to get the last 30 days mail, even if it has already been downloaded elsewhere.
   ...
   ## Example
     ...
     Download maximum 2 emails from the email account and also save the raw undecoded message.
       ...
       iex(1)> Pop3.download(%{"max" => 2, "raw" => true, "username" => "hendrik.lorentz@gmail.com", "password" => "secret", "output" => "mailbox"})

   """
   @spec download(%{required(String.t) => String.t | boolean | non_neg_integer}) :: {:ok, integer} | {:error, String.t}
   def download(params) do
     epop_options = %Pop3.EpopDownloader.Options{
     ...
     Pop3.EpopDownloader.download(epop_options)
   end
   ...
   ## Example
     ...
     Retrieve email via epop_client and lookup headers 'Data', 'Subject' and 'From' and close the connection.
       ...
       iex(6)> Pop3.header_lookup(header_list, "Subject")
       "Solution Skolem problem"
       iex(7)> Pop3.header_lookup(header_list, "From")
       "Hendrik Lorentz <hendrik.lorentz@gmail.com>"
       iex(8)> Pop3.header_lookup(header_list, "Date")
   ...
   def header_lookup(header_list, header_name) do
      Pop3.Header.lookup(header_list, header_name)
   end
   ...
   ## Examples
     ...
     Russian subject
       ...
       iex> Pop3.decode_words("=?koi8-r?Q?Fwd:_=E4=CF=CD=C1=DB=CE=C5=C5_=DA=C1=C4=C1=CE=C9=C5_?=")
       [{"koi8-r",
     ...
     French (Réception) and ascii text email address
       ...
       iex(1)> Pop3.decode_words("=?iso-8859-1?Q?R=E9ception_Fayence?= <reception.fayence@acme.com>")
       [{"iso-8859-1",
     ...
     Chinese
       ...
       iex(1)> Pop3.decode_words("chinese")
       [{"us-ascii", "chinese"}]

   """
   @spec decode_words(String.t) :: list({String.t, binary})
   def decode_words(text) do
      Pop3.WordDecoder.decode_text(text)
   end

   @doc ~S"""
   Decode multipart, base64 and quoted-printable text. Returned is a list of Pop3.Part structs.
   ...
   ## Example
     ...
     Retrieve email via epop_client and decode body and close the connection.
       ...
       iex(4)> {:ok, mail_content} = :epop_client.bin_retrieve(client, 1)
       iex(5)> {:message, header_list, body_content } = :epop_message.bin_parse(mail_content)
       iex(6)> Pop3.decode_body_content(header_list, body_content)
       [%Pop3.Part{boundary: "--_com.android.email_1191110031918720",
         charset: "utf-8",
   ...
   """
   @spec decode_body_content(list({:header, String.t, String.t}), String.t) :: list(Pop3.Part.t)
   def decode_body_content(header_list, body_content) do
      Pop3.Handler.decode_body_content(header_list, body_content)
   end
   ...
   ## Example
     ...
     Decode message with nested multipart content.
       ...
       ...(1)>
       ...(1)> """
       ...(2)> message |> String.replace("\n", "\r\n") |> Pop3.decode_body("multipart/related; boundary=\"----=_NextPart_000_0004_01D0C782.71A41900\"", "bit7", "")
       [%Pop3.Part{boundary: "----=_NextPart_001_0005_01D0C782.71A8D3F0",
         charset: "iso-8859-1",
        ...
        %Pop3.Part{boundary: "----=_NextPart_000_0004_01D0C782.71A41900",
         charset: "us-ascii",
   ...
   '''
   @spec decode_body(String.t, String.t, String.t, String.t) :: list(Pop3.Part.t)
   def decode_body(body_text, content_type \\ "text/plain; charset=us-ascii", encoding \\ "7bit", disposition \\ "inline") do
      Pop3.Body.decode_body(body_text, content_type, encoding, disposition)
   end
   ...
   ## Example
     ...
     Decode simple.eml and write result in the testoutput directory.
       ...
       iex(1)> Pop3.decode_raw_file("test/pop3/fixtures/simple.eml", "testoutput")
   ...
   def decode_raw_file(filename, output_dir) do
      ...
      case :file.read_file(filename) do
         {:ok, mail_content}  -> mail_content |> Pop3.EpopDownloader.parse_process_and_store(1, nil, false, output_dir)
