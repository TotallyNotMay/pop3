defmodule Pop3.CLI do
  alias Pop3.EpopDownloader

   @moduledoc "Commandline interface for downloading emails and storing them on disk."

   defp usage_text do
     """
     usage: pop3_downloader [--username=[recent:]USERNAME]
   ...
   defp show_error([]), do: IO.puts(:stderr, "Type 'pop3_downloader --help' for more information.")

   # "print unknown option"
   defp show_error([{option, _} | tail]) do
      IO.puts(:stderr, "pop3_downloader: Unknown option '" <> to_string(option) <> "'")
      show_error tail
   end

   # "print unknown parameter"
   defp show_error([argument | tail]) do
      IO.puts(:stderr, "pop3_downloader: Unknown parameter '" <> argument <> "'")
      show_error tail
   end
